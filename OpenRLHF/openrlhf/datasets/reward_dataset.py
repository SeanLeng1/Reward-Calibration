...
def human_assistant_format(data, chosen_key, rejected_key):
    human = 'Human: {prompt}'
    ...
    for msg in chosen:
        if msg['role'] == 'system':
            formatted_chosen += human.format(prompt = msg['content'])
            formatted_chosen = formatted_chosen.rstrip('\n') + '\n\n'
        elif msg['role'] == 'user':
            if formatted_chosen:
                formatted_chosen += msg['content']
                formatted_chosen = formatted_chosen.rstrip('\n') + '\n'
            else:
                formatted_chosen += human.format(prompt = msg['content'])
            formatted_chosen = formatted_chosen.rstrip('\n') + '\n'
        else:
            formatted_chosen += assistant.format(response = msg['content'])
            formatted_chosen = formatted_chosen.rstrip('\n') + '\n'
    formatted_chosen = formatted_chosen.rstrip('\n')

    for msg in rejected:
        if msg['role'] in ['user', 'system']:
            formatted_rejected += human.format(prompt = msg['content'])
            formatted_rejected = formatted_rejected.rstrip('\n') + '\n'
        else:
            formatted_rejected += assistant.format(response = msg['content'])
            formatted_rejected = formatted_rejected.rstrip('\n') + '\n'
    formatted_rejected = formatted_rejected.rstrip('\n')
